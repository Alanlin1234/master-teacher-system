"""数据库服务层：预置种子数据与 CRUD 实现"""
from typing import Optional, List, Dict, Any
from sqlalchemy import select, delete
from sqlalchemy.ext.asyncio import AsyncSession
from app.database import User, Teacher, SynthesizedTeacher, StudioJob, TeachingStrategy
...
class UserService:
    ...
    @staticmethod
    async def create(db: AsyncSession, username: str, password_hash: str, role: str = "user", portal_role: str = "student", email: str = "") -> User:
        user = User(username=username, password_hash=password_hash, role=role, portal_role=portal_role, email=email)
        db.add(user)
        await db.commit()
    ...
    async def ensure_default_users(db: AsyncSession):
        ...
        # 预置演示账号
        defaults = [
            User(username="demo_student", role="user", portal_role="student", password_hash="123456", email="student@xunzhi.edu"),
            User(username="demo_teacher", role="user", portal_role="teacher", password_hash="123456", email="teacher@xunzhi.edu"),
            User(username="admin", role="admin", portal_role="teacher", password_hash="admin123", email="admin@xunzhi.edu"),
        ]
        for u in defaults:
            db.add(u)
        await db.commit()
...
class TeacherService:
    ...
    @staticmethod
    async def list_all(db: AsyncSession) -> List[Teacher]:
        stmt = select(Teacher).where(Teacher.is_published == True)
        res = await db.execute(stmt)
        return list(res.scalars().all())

    @staticmethod
    async def get_by_id(db: AsyncSession, teacher_id: str) -> Optional[Teacher]:
    ...
    async def seed_teachers(db: AsyncSession):
        ...
        if res.scalar_one_or_none():
            return
        for item in TeacherService.DEFAULT_TEACHERS:
            t = Teacher(**item)
            db.add(t)
        await db.commit()

class SynthTeacherService:
    @staticmethod
    async def list_all(db: AsyncSession) -> List[SynthesizedTeacher]:
        stmt = select(SynthesizedTeacher).order_by(SynthesizedTeacher.created_at.desc())
        res = await db.execute(stmt)
        return list(res.scalars().all())

    @staticmethod
    async def create(db: AsyncSession, id: str, name: str, source_teachers: list, recipe: dict, mode: str, agent_prompt: str, critic_log: list) -> SynthesizedTeacher:
        st = SynthesizedTeacher(
            id=id,
            name=name,
